...
pygame.display.set_caption("TicTacToeGUI")
Running = True
...
# To check whether the boxes are filled or not
a = b = c = d = e = f = g = h = i = True
# Keeps track of the X and O positions
...
def press_space():
    press_space_a = gamefont.render(
        'Press Space to Exit', True, (255, 255, 255))
    screen.blit(press_space_a, (50, 650))


def tie_case():
    tie_case = gamefont.render("It's a tie", True, (255, 255, 255))
    screen.blit(tie_case, (135, 600))
...
while Running:
    ...
    for item in board:
        if item[0] in [1, 2] and item[1] in [1, 2] and item[2] in [1, 2]:
            full += 1
